...
import gui_inline_1f from "./gui_inline_1f";
import gui_inline_2f from "./gui_inline_2f";
import gui_inline_col from "./gui_inline_col";
...
export default ShaderBoy.gui = {
  ...
  init() {
    ...
    this.knobUniformFS = "\n";
    this.midiUniformFS = "\n";
    this.inline1fUniformFS = "\n";
    this.inline2fUniformFS = "\n";
    this.inline3fUniformFS = "\n";
  ...
  setupMouse() {
    ...
    function calcMouseX(ev) {
      ...
      return Math.floor(
        ((ev.clientX - rect.left) / (rect.right - rect.left)) * c.width
      );
    }
    ...
    function calcMouseY(ev) {
      ...
      return Math.floor(
        c.height -
          ((ev.clientY - rect.top) / (rect.bottom - rect.top)) * c.height
      );
    }
    ...
    document.addEventListener("touchstart", (ev) => {
      onMouseDown(touchToMouse(ev));
    });
    document.addEventListener("touchend", (ev) => {
      onMouseUp(touchToMouse(ev));
    });
    document.addEventListener("touchmove", (ev) => {
      onMouseMove(touchToMouse(ev));
    });
    ...
    document.addEventListener("wheel", (ev) => {
      ...
      if (ShaderBoy.isEditorHidden || (ShaderBoy.isSplited && onCanvas(ev))) {
        this.iWheelCumulative[0] += ev.deltaX * 0.01;
        this.iWheelCumulative[1] += ev.deltaY * 0.01;
  ...
  showAuth() {
    ...
    const authContentClassList =
      document.getElementById("auth-content").classList;
  ...
  setupCanvasKeyboardControl() {
    ...
    // エディタにフォーカスがある場合、キャンバスのキーイベントを無効化
    editorElement.addEventListener("focusin", () => {
      canvas.setAttribute("tabindex", "-1"); // キャンバスがフォーカスを受け取れないようにする
    });

    // エディタからフォーカスが外れたとき、キャンバスでキー入力を再び受け付ける
    editorElement.addEventListener("focusout", () => {
      // 他の入力エリアにフォーカスが移っていない場合のみ
